...
import { Form, Button, Row, Col, DatePicker, Radio, message } from 'antd'
...
const Filter = ({
  ...
  const onChange = (dates, dateStrings) => {
    ...
    if (dates.length === 0) {
      console.log('没选时间')
      dispatch({
        type: 'queryBatTxnTimeout/disabledDate',
        payload: {
          disabledDate: () => {
            // Can not select days before today and today
            return false
    ...
    /*    let fields = getFieldsValue()
        fields.txnTime = [moment('2017-01-01'), moment('2017-03-01')]
    ...
    <Form.Item>
      <Row gutter={24} style={{ overflow: 'hidden' }}>
        <Col xl={{ span: 12 }} md={{ span: 12 }} {...BatchTitle}>
          <FormItem {...ColProps} label="交易时间：" {...formItemLayoutd} >
            {
        ...
        <Col xl={{ span: 12 }} md={{ span: 12 }} {...BatchTitle}>
          <FormItem {...ColProps} label="处理状态：" {...formItemLayoutd} >
            {
              getFieldDecorator('dealSts', {
                ...
                <RadioButton value="43">已同步</RadioButton>
                <RadioButton value="42">同步中</RadioButton>
                <RadioButton value="41">未处理</RadioButton>
                <RadioButton value="00">忽略</RadioButton>
              </RadioGroup>)}
          </FormItem>
        </Col>
