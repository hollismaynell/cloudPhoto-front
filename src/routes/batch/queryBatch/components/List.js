...
const List = ({ onDeleteItem, dispatch, pagination, isMotion, ...tableProps }) => {
  const handleMenuClick = (record, e) => {
    if (e.key === '1') {
      dispatch({
        type: 'jobLogDetail/queryOne',
        payload: {
          trcNo: record.trcNo,
          trcDtlNo: record.trcDtlNo,
        },
      })
    }
  }
  ...
  const columns = [
    {
      ...
      // render: (text, record) => {
      //   // return <div href={record.id}>查询</div>
      //   return <DropOption onMenuClick={e => handleMenuClick(record, e)} menuOptions={[{ key: '1', name: '修改' }, { key: '3', name: '删除' }]} />
      // },
    },
  ]
  ...
  const title = () => { return <h3>批量收款交易记录</h3> }
  return (
    <div>
      <Table
        {...tableProps}
